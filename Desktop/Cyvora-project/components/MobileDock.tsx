...
export default function MobileDock() {
  ...
  useEffect(() => {
    ...
    const read = () => { try { const value = JSON.parse(window.localStorage.getItem('cyvora.dock') || JSON.stringify(defaults)); if (Array.isArray(value) && value.length >= 3) setIds(value.filter((id) => catalog[id]).slice(0, 5)); } catch { setIds(defaults); } };
    updateViewport();
    read();
    media.addEventListener('change', updateViewport);
    window.addEventListener('cyvora:dock-updated', read);
    return () => { media.removeEventListener('change', updateViewport); window.removeEventListener('cyvora:dock-updated', read); };
  }, []);
  if (!isMobile) return null;
  if (pathname.startsWith('/unlock')) return null;
  return <nav className="cyvora-mobile-dock" aria-label="Mobile navigation" style={{ gridTemplateColumns: `repeat(${ids.length}, minmax(0, 1fr))` }}>{ids.map((id) => { const item = catalog[id]; const active = item.href === '/' ? pathname === '/' : pathname === item.href || pathname.startsWith(`${item.href}/`); return <Link key={id} href={item.href} className={`cyvora-mobile-dock-item ${active ? 'is-active' : ''} ${id === 'executive' ? 'is-primary' : ''}`}><span className="text-base leading-none">{item.glyph}</span><span className="mt-1 block truncate text-[9px]">{item.label}</span></Link>; })}</nav>;
}
