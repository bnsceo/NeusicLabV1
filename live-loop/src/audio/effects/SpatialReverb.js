export class SpatialReverb {
  constructor(context) {
    ...
    this.wet.gain.value = .24; this.dry.gain.value = 0; this.freezeGain.gain.value = 1;
    this.convolver.buffer = this.makeImpulse(this.size,2.6);
    // This is a send/return effect. The lane already has a direct dry path,
    // so the effect return must contain wet signal only.
    this.input.connect(this.convolver);
    this.convolver.connect(this.tone);
    this.tone.connect(this.wet);
    this.wet.connect(this.freezeGain);
  ...
  setMix(value) {const mix=Math.max(0,Math.min(1,value));this.wet.gain.setTargetAtTime(mix,this.context.currentTime,.03);this.dry.gain.setTargetAtTime(0,this.context.currentTime,.01);}
